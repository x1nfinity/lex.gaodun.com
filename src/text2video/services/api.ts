...
function getApiKeyForModel(modelId: string): string | undefined {
    switch (modelId) {
        case 'deepseek':
            return import.meta.env.VITE_REACT_APP_DEEPSEEK_TOKEN;
        case 'doubao':
            return import.meta.env.VITE_REACT_APP_DOUBAO_TOKEN;
        case 'qwen':
            return import.meta.env.VITE_REACT_APP_QWEN_TOKEN;
        case 'chatgpt':
            return import.meta.env.VITE_REACT_APP_OPENAI_TOKEN;
        default:
            return undefined;
    }
}
...
谐音记忆: ${scriptData.mnemonic_phrase}
  `.trim();
    ...
    console.log('Headers:', {
        Authorization: `Bearer ${import.meta.env.VITE_REACT_APP_ALIYUN_TOKEN ? '***已配置***' : '未配置'}`,
        'Content-Type': 'application/json',
        'X-DashScope-Async': 'enable',
    });
