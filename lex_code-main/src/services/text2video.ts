...
export class Text2VideoService {
  ...
  async generateVideo(
    ...
    try {
      ...
      callbacks?.onProgress?.('脚本生成完成，开始生成视频...');
      
      // 步骤2: 生成视频
      callbacks?.onStepChange?.(GenerationStep.GENERATING_VIDEO);
      callbacks?.onProgress?.('正在创建视频生成任务...');
  ...
  private async pollVideoStatus(taskId: string, callbacks?: ProgressCallback): Promise<string> {
    ...
    while (attempts < this.config.maxPollingAttempts) {
      try {
        ...
        if (status.output?.task_status === 'SUCCEEDED') {
          ...
          if (status.output.video_url) {
            ...
            // 检查是否有其他可能的URL字段
             ...
             const possibleUrl = outputAny?.url || 
                               outputAny?.download_url ||
                               outputAny?.file_url;
        ...
        callbacks?.onProgress?.(`视频生成中... (状态: ${statusText}, 预计还需 ${remainingTime} 秒)`);
